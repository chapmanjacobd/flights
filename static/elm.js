...
var $author$project$Main$AirportResponse = function (a) {
	return {$: 'AirportResponse', a: a};
};
var $krisajenkins$remotedata$RemoteData$Loading = {$: 'Loading'};
var $author$project$Route$Root = {$: 'Root'};
var $author$project$Main$Airport = F8(
	function (uFrom, flyFrom, flyTo, distance, cityFrom, price, cityTo, kiwiLink) {
		return {cityFrom: cityFrom, cityTo: cityTo, distance: distance, flyFrom: flyFrom, flyTo: flyTo, kiwiLink: kiwiLink, price: price, uFrom: uFrom};
	});
...
var $author$project$Main$airportPatternDecoder = A2(
	$NoRedInk$elm_json_decode_pipeline$Json$Decode$Pipeline$hardcoded,
	'https://www.kiwi.com/deep?from=',
	A4(
					...
					A4(
						...
						A4(
							...
							A4(
								...
								$elm$json$Json$Decode$string,
								'',
								$elm$json$Json$Decode$succeed($author$project$Main$Airport)))))))));
var $author$project$Main$airportDecoder = $elm$json$Json$Decode$list($author$project$Main$airportPatternDecoder);
var $author$project$Main$apiUrl = 'http://localhost:3006';
...
var $author$project$Main$changeRouteTo = F3(
	function (route, model, navKey) {
		switch (route.$) {
			case 'NotFound':
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 'Root':
				return _Utils_Tuple2(
					model,
					A2($author$project$Route$replaceUrl, navKey, $author$project$Route$Root));
			default:
				var airportid = route.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							airport: $krisajenkins$remotedata$RemoteData$Loading,
							tableQuery: '',
							tableState: $billstclair$elm_sortable_table$Table$initialSort('')
						}),
					A3($ohanhi$remotedata_http$RemoteData$Http$get, $author$project$Main$apiUrl + ('/getFlightRoutes?k=' + airportid), $author$project$Main$AirportResponse, $author$project$Main$airportDecoder));
		}
	});
...
var $author$project$Route$ToAirport = function (a) {
	return {$: 'ToAirport', a: a};
};
...
var $author$project$Route$parser = $elm$url$Url$Parser$oneOf(
	_List_fromArray(
		[
			...
			$author$project$Route$ToAirport,
			A2(
				$elm$url$Url$Parser$slash,
				$elm$url$Url$Parser$s('toAirport'),
		...
		return A2(
			...
			A2(
				...
				_Utils_update(
					url,
					{
						path: A3($elm$core$String$replace, basePath, '', url.path)
					})));
	});
...
var $author$project$SearchData$searchData = _List_fromArray(
	[
		A3($author$project$SearchData$Search, 1, 'Mexico CityDistrito FederalMX[ Mexico City    Ciudad de M    Mexiko-Stadt    Ciudad de México    Mexico    Ciudad de México    Мехико    墨西哥城    مدينة مكسيكو    মেক্সিকো সিটি    Πόλη του Μεξικού    मेक्सिको नगर    Mexikóváros    Kota Meksiko    Città del Messico    メキシコシティ    멕시코시티    Mexico-Stad    Meksyk    Mexico City    Meksiko    Thành phố México ][ TLC    MEX ][ MMCB    MMMX    MMPB    MMQT    MMTO    MMJC ]', 'Mexico City (Distrito Federal MX)'),
		A3($author$project$SearchData$Search, 2, 'MumbaiMaharashtraIN[ Mumbai    Bombay    Mumbai    Bombay    Bombay    Bombaim    Мумбаи    孟买    مومباي    মুম্বই    Μουμπάι    मुम्बई    Mumbai    Mumbai    Mumbai    ムンバイ    뭄바이    Bombay    Mumbaj    Bombay    Mumbai    Mumbai ][ DIU    PNQ    BOM ][ VABB    VAOZ    VAPO    VASD    VASU    VAJJ ]', 'Mumbai (Maharashtra IN)'),
		A3($author$project$SearchData$Search, 3, 'Sao PauloSão PauloBR[ São Paulo    Sao Paulo|Sio Paulo    São Paulo    São Paulo    São Paulo    São Paulo    Сан-Паулу    聖保羅    ساو باولو    সাও পাওলো    Σάο Πάολο    साओ पाउलो    São Paulo    São Paulo    San Paolo    サンパウロ    상파울루    São Paulo    São Paulo    São Paulo    São Paulo    São Paulo ][ SOD    VCP    CGH ][ SBGR    SBKP    SBSJ    SBSP    SBMT    SDNI    SDTB ]', 'Sao Paulo (São Paulo BR)'),
		A3($author$project$SearchData$Search, 4, 'ShanghaiShanghaiCN[ Shanghai    Shanghai    Shanghái    Shanghai    Xangai    Шанхай    上海    شانغهاي    সাংহাই    Σαγκάη    शंघाई    Sanghaj    Shanghai    Shanghai    上海市    상하이 시    Shanghai    Szanghaj    Shanghai    Şanghay    Thượng Hải ][ NTG    WUX    SHA ][ ZSHC    ZSNB    ZSPD    ZSSS    ZSWX ]', 'Shanghai (Shanghai CN)'),
		A3($author$project$SearchData$Search, 5, 'Buenos AiresCiudad de Buenos AiresAR[ Buenos Aires    Buenos Aires    Buenos Aires    Buenos Aires    Buenos Aires    Буэнос-Айрес    布宜諾斯艾利斯    بوينس آيرس    বুয়েনোস আইরেস    Μπουένος Άιρες    ब्यूनस आयर्स    Buenos Aires    Buenos Aires    Buenos Aires    ブエノスアイレス    부에노스아이레스    Buenos Aires    Buenos Aires    Buenos Aires    Buenos Aires    Buenos Aires ][ AEP    EZE ][ SABE    SAEZ    SABE    SADQ    SADZ ]', 'Buenos Aires (Ciudad de Buenos Aires AR)'),
		A3($author$project$SearchData$Search, 6, 'KarachiSindPK[ Karachi    Karatschi    Karachi    Karachi    Carachi    Карачи    卡拉奇    كراتشي    করাচী    Καράτσι    कराची    Karacsi    Karachi    Karachi    カラチ    카라치    Karachi    Karaczi    Karachi    Karaçi    Karachi ][ HDD    KHI ][ OPKC    OPNH    OPSN    OPKK    OPMR    OPSF ]', 'Karachi (Sind PK)'),
		A3($author$project$SearchData$Search, 7, 'MoscowMoskvaRU[ Moscow    Moskva    Moskau    Moscú    Moscou    Moscovo    Москва    莫斯科    موسكو    মস্কো    Μόσχα    मास्को    Moszkva    Moskwa    Mosca    モスクワ    모스크바    Moskou    Moskwa    Moskva    Moskova    Moskva ][ OSF    SVO ][ UUBW    UUDD    UUEE    UUMO    UUWW    UUBM    UUMO ]', 'Moscow (Moskva RU)'),
		A3($author$project$SearchData$Search, 8, 'TokyoTokyoJP[ Tokyo    Präfektur Tokio    Tokio    Tokyo    Tóquio    Токио    東京都    طوكيو    টোকিও    Τόκιο    टोक्यो    Tokió    Tokyo    Tokyo    東京都    도쿄도    Tokio    Tokio    Tokyo prefektur    Tokyo    Tokyo ][ IBR    HND ][ RJAA    RJAH    RJTF    RJTO    RJTT    RJTF    RJTK    RJTL ]', 'Tokyo (Tokyo JP)'),
		A3($author$project$SearchData$Search, 9, 'New YorkNew YorkUS[ New York    New York-Newark    New York City    Nueva York    New York    Nova Iorque    Нью-Йорк    纽约    نيويورك    নিউ ইয়র্ক সিটি    Νέα Υόρκη    न्यूयॉर्क नगर    New York    New York City    New York    ニューヨーク    뉴욕    New York City    Nowy Jork    New York    New York    Thành phố New York ][ WST    HPN    LGA ][ KEWR    KHPN    KJFK    KLGA    KSWF    KCDW    KLDJ    KTEB ]', 'New York (New York US)'),
		A3($author$project$SearchData$Search, 10, 'DelhiDelhiIN[ Delhi    Delhi    Delhi    Delhi    Deli    Дели    德里    دلهي    দিল্লি    Δελχί    दिल्ली    Delhi    Delhi    Delhi    デリー    델리    Delhi    Delhi    Delhi    Delhi    Delhi ][ AIP    DED    DEL ][ VICG    VIDN    VIDP    VA2B    VIDD    VIDX ]', 'Delhi (Delhi IN)')
	]);
var $author$project$Main$searchCategories = $elm$core$List$concat(
	_List_fromArray(
		[
			_List_fromArray(
			[
				$author$project$Selectize$divider('Cities')
			]),
...
var $author$project$Main$init = F3(
	function (flags, url, navKey) {
		...
		var model = {
			airport: $krisajenkins$remotedata$RemoteData$NotAsked,
			flags: flags,
			key: navKey,
			route: A2($author$project$Route$fromUrl, basePath, url),
		...
		return A3(
			...
			A2($author$project$Route$fromUrl, basePath, url),
			model,
			navKey);
	});
...
var $author$project$Main$SelectTextfieldAirport = function (a) {
	return {$: 'SelectTextfieldAirport', a: a};
};
	...
	function (msg, model) {
		...
		while (true) {
			...
			switch (msg.$) {
				...
				case 'AirportResponse':
					var data = msg.a;
					var $temp$msg = $author$project$Main$ClickedLink(
				...
				case 'ChangedUrl':
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								route: A2($author$project$Route$fromUrl, flags.basePath, url)
							}),
						$elm$core$Platform$Cmd$none);
				case 'ClickedLink':
				...
				case 'SelectTextfieldAirport':
					...
					if (newSelection.$ === 'Nothing') {
						return _Utils_Tuple2(
							...
							A3($ohanhi$remotedata_http$RemoteData$Http$get, $author$project$Main$apiUrl + ('/searchOrigin?input=' + model.textfieldMenu.query), $author$project$Main$SearchResponse, $author$project$Main$searchDecoder));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
							...
							A3(
								...
								$author$project$Main$apiUrl + ('/getFlightRoutes?k=' + $elm$core$String$fromInt(
									A2(
										$elm$core$Maybe$withDefault,
										0,
										A2(
											...
											newSelection)))),
								$author$project$Main$AirportResponse,
								$author$project$Main$airportDecoder));
					}
				default:
					var selectizeMsg = msg.a;
					var _v3 = A4($author$project$Selectize$update, $author$project$Main$SelectTextfieldAirport, model.textfieldSelection, model.textfieldMenu, selectizeMsg);
					var newMenu = _v3.a;
					var menuCmd = _v3.b;
					var maybeMsg = _v3.c;
...
var $author$project$Main$SetTableQuery = function (a) {
	return {$: 'SetTableQuery', a: a};
};
var $author$project$Main$airportTitle = 'te';
...
var $author$project$Main$viewDollars = function (dollars) {
	return '$' + $elm$core$String$fromInt(dollars);
};
var $author$project$Main$dollarColumn = F2(
	function (name, toDollars) {
		return $billstclair$elm_sortable_table$Table$customColumn(
			{
				name: name,
				sorter: $billstclair$elm_sortable_table$Table$increasingOrDecreasingBy(toDollars),
				viewData: function (data) {
					return $author$project$Main$viewDollars(
						toDollars(data));
				}
			});
	});
...
var $author$project$Main$viewkiwilinks = function (kiwilinks) {
	return A2(
		$billstclair$elm_sortable_table$Table$HtmlDetails,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$href(kiwilinks),
								$elm$html$Html$Attributes$target('_blank')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('Check price')
							]))
					]))
			]));
};
var $author$project$Main$kiwilinkColumn = F2(
	function (name, tokiwilinks) {
		return $billstclair$elm_sortable_table$Table$veryCustomColumn(
			{
				name: name,
				sorter: $billstclair$elm_sortable_table$Table$decreasingBy(tokiwilinks),
				viewData: function (data) {
					return $author$project$Main$viewkiwilinks(
						tokiwilinks(data));
				}
			});
	});
...
var $author$project$Main$config = $billstclair$elm_sortable_table$Table$config(
	{
		columns: _List_fromArray(
			[
				...
				A2(
				$author$project$Main$dollarColumn,
				'Lowest Price',
				function ($) {
					return $.price;
				}),
				...
				A2(
				$author$project$Main$kiwilinkColumn,
				'Kiwi',
				function (rec) {
					return rec.kiwiLink + (rec.flyFrom + ('&to=' + rec.flyTo));
				})
			]),
		toId: function ($) {
			return $.cityFrom;
...
var $author$project$Main$view = function (model) {
	...
	return {
				...
				_List_fromArray(
					[
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('See all flights to an airport: ')
											])),
										A2(
										$elm$html$Html$div,
										_List_fromArray(
						...
						_List_fromArray(
							[
								function () {
								var _v0 = model.airport;
								switch (_v0.$) {
									case 'Loading':
										return $elm$html$Html$text('Loading airport data, please stand by...');
									case 'Success':
										var airport = _v0.a;
										var acceptableAirport = A2(
										...
										return A2(
											...
											_List_fromArray(
												[
													...
													$elm$html$Html$text('Filter by City: '),
													A2(
													$elm$html$Html$input,
													_List_fromArray(
														[
															$elm$html$Html$Events$onInput($author$project$Main$SetTableQuery)
														]),
													_List_Nil),
													A3($billstclair$elm_sortable_table$Table$view, $author$project$Main$config, tableState, acceptableAirport)
												]));
									case 'Failure':
										var error = _v0.a;
										return $elm$html$Html$text(
											'Oh noes, airport loading failed with error: ' + $author$project$HttpExtra$errorToString(error));
									default:
										return A2(
											$elm$html$Html$div,
											_List_Nil,
											_List_fromArray(
												[
													...
													_List_fromArray(
														[
															$elm$html$Html$text('Please select a city')
														])),
													A2(
													$elm$html$Html$div,
													_List_Nil,
													_List_fromArray(
														[
															...
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															$elm$html$Html$text('This website will help you search backwards with historical flight prices and routes.'),
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															$elm$html$Html$text('It\'s useful for if you are unsure whether a flight is a good price, or if you want to route a 3+ destination trip in a cheap way.'),
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															$elm$html$Html$text('I\'ve used this data when planning my own trips and confirming whether or not a deal is a good price.'),
															A2($elm$html$Html$br, _List_Nil, _List_Nil),
															$elm$html$Html$text('But know that the data is not 100% accurate for tickets that cost more than $800.')
														]))
												]));
								}
							}()
		...
		title: $author$project$Main$airportTitle + 'UNLI Flights'
	};
};
...
_Platform_export({'Main':{'init':$author$project$Main$main(
	A2(
		...
		A2($elm$json$Json$Decode$field, 'basePath', $elm$json$Json$Decode$string)))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Main.Airport":{"args":[],"type":"{ uFrom : String.String, flyFrom : String.String, flyTo : String.String, distance : Basics.Int, cityFrom : String.String, price : Basics.Int, cityTo : String.String, kiwiLink : String.String }"},"Selectize.Msg":{"args":["a"],"type":"Selectize.Selectize.Msg a"},"SearchData.Search":{"args":[],"type":"{ id : Basics.Int, name : String.String, displayname : String.String }"},"Url.Url":{"args":[],"type":"{ protocol : Url.Protocol, host : String.String, port_ : Maybe.Maybe Basics.Int, path : String.String, query : Maybe.Maybe String.String, fragment : Maybe.Maybe String.String }"},"RemoteData.WebData":{"args":["a"],"type":"RemoteData.RemoteData Http.Error a"},"Selectize.Selectize.Heights":{"args":[],"type":"{ entries : List.List Basics.Float, menu : Basics.Float }"}},"unions":{"Main.Msg":{"args":[],"tags":{"SetTableQuery":["String.String"],"SetTableState":["Table.State"],"TextfieldMenuMsg":["Selectize.Msg SearchData.Search"],"SelectTextfieldAirport":["Maybe.Maybe SearchData.Search"],"SearchResponse":["RemoteData.WebData (List.List SearchData.Search)"],"AirportResponse":["RemoteData.WebData (List.List Main.Airport)"],"ChangedUrl":["Url.Url"],"ClickedLink":["Browser.UrlRequest"]}},"Http.Error":{"args":[],"tags":{"BadUrl":["String.String"],"Timeout":[],"NetworkError":[],"BadStatus":["Basics.Int"],"BadBody":["String.String"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"List.List":{"args":["a"],"tags":{}},"Maybe.Maybe":{"args":["a"],"tags":{"Just":["a"],"Nothing":[]}},"Selectize.Selectize.Msg":{"args":["a"],"tags":{"NoOp":[],"OpenMenu":["Selectize.Selectize.Heights","Basics.Float"],"CloseMenu":[],"FocusTextfield":[],"BlurTextfield":[],"PreventClosing":["Basics.Bool"],"SetQuery":["String.String"],"SetMouseFocus":["Maybe.Maybe a"],"Select":["a"],"SetKeyboardFocus":["Selectize.Selectize.Movement","Basics.Float"],"SelectKeyboardFocusAndBlur":[],"ClearSelection":[]}},"Url.Protocol":{"args":[],"tags":{"Http":[],"Https":[]}},"RemoteData.RemoteData":{"args":["e","a"],"tags":{"NotAsked":[],"Loading":[],"Failure":["e"],"Success":["a"]}},"Table.State":{"args":[],"tags":{"State":["String.String","Basics.Bool"]}},"String.String":{"args":[],"tags":{"String":[]}},"Browser.UrlRequest":{"args":[],"tags":{"Internal":["Url.Url"],"External":["String.String"]}},"Basics.Bool":{"args":[],"tags":{"True":[],"False":[]}},"Basics.Float":{"args":[],"tags":{"Float":[]}},"Selectize.Selectize.Movement":{"args":[],"tags":{"Up":[],"Down":[],"PageUp":[],"PageDown":[]}}}}})}});}(this));